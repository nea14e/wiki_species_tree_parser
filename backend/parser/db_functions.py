...
class DbFunctions:
    ...
    def init_db(is_test: bool = False):
        ...
        Logger.print("\nХранимка по расчёту процента от двух чисел (служебная): перенакатываем...")
        DbExecuteNonQuery.execute_file("init_db", os.path.join("init_db", "functions", "percent_of.sql"))
        Logger.print("\nХранимка по расчёту цвета от двух чисел (служебная): перенакатываем...")
        DbExecuteNonQuery.execute_file("init_db", os.path.join("init_db", "functions", "percent_of_to_color.sql"))
