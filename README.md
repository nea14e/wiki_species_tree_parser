...
Этот этап можно выполнить в любое время, при этом он будет использовать только те записи, которые к тому моменту будут уже распарсены на этапе 2,
а когда этап 2 пройдут все записи, то можно вызвать этот этап снова и ничего при этом не сломается.
...
Этот этап можно выполнить в любое время, при этом он будет использовать только те записи, которые к тому моменту будут уже обновлены на этапе 3,
а когда этап 3 пройдут все записи, то можно вызвать этот этап снова и ничего при этом не сломается.
...
- API для frontend (URL-адреса начинаются с префикса `API/`):
  ...
  - GET http://127.0.0.1:8000/api/get_tree_by_id/222 - выдаёт дерево, раскрытое на записи с нужным id (уровни до него и все его прямые потомки на разных уровнях)
  - GET http://127.0.0.1:8000/api/search_by_words/Позвоноч - поиск по названию. Запрос должен быть на языке браузера пользователя либо на латыни. Ищется по началу названия. Выдаёт список.
